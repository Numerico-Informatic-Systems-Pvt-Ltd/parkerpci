...
class Image_Text_Widget extends WP_Widget
{
	...
	public function widget($args, $instance)
	{
		...
		if($image_position === 'above_text')
		{
			...
			$html .= '<a href="'.$href.'"><img  src="'.$image[0].'"  alt="'.$alt.'"'.$image_align.' /></a>';
			$html .= str_replace('h2','h3',$title);
		...
		elseif($image_position === 'below_text')
		{
			...
			$html .= ($href !== '' ? '<a href="'.$href.'"'.($instance['link_target'] === 'new' ? ' target="_blank"' : '').'>' : '').'<img class="image-text-widget-image" src="'.$image[0].'" width="'.$width.'" height="'.$height.'"  alt="'.$alt.'"'.$image_align.' />'.($href !== '' ? '</a>' : '');
		}
		$html .= '</div></li>';
